...
function sim_mean_strength_switching(vec_data, vec_res, pref; mu, sd)
    ...
    for model in 1:nmodel
        ...
        for i in 1:datasize-1
            ...
            # discretise jacobian matrix
            jmat_true_discrete = get_jmat_discrete(jmat_true_scaled)
            ...
            # true mean strength
            mean_strength_true[i] = mean(jmat_true_discrete)

            vec_jmat_gpr[model][:,:,i] .= jmat_mean
            vec_jmat_true[model][:,:,i] .= jmat_true_discrete
        end
...
function stats_sim_jmat(res_mean_strength, vec_res, vec_jmat_scaled)
    ...
    vec_diff_gpr = copy(vec_mean_strength_before_gpr)
    vec_diff_true = copy(vec_mean_strength_before_gpr)

    # accuracy for mean interaction
    vec_sign_diff_mean = zeros(nmodel)
    # accuracy for interaction changes
    vec_cor_diff = copy(vec_sign_diff_mean)
    ...
    for model in 1:nmodel
        ...
        #accuracy_sign_diff = zeros(datasize-1)

        for iter in 1:datasize-1
            mean_strength_before_gpr[iter] = mean(vec_res[model].jmat[:,:,iter])
            mean_strength_before_true[iter] = mean(vec_jmat_scaled[model][:,:,iter])

            #@. sign_diff_gpr[:,:,iter] = sign(vec_jmat_gpr[:,:,iter] - vec_res[model].jmat[:,:,iter])
            #@. sign_diff_true[:,:,iter] = sign(vec_jmat_true[:,:,iter] - vec_jmat_scaled[model].jmat[:,:,iter])
            #accuracy_sign_diff[iter] = mean(sign_diff_gpr[:,:,iter] .== sign_diff_true[:,:,iter])
        end

        vec_mean_strength_before_gpr[model] = mean_strength_before_gpr
        vec_mean_strength_before_true[model] = mean_strength_before_true

        #sign_diff_gpr = sign.(vec_jmat_gpr[model] - vec_res[model].jmat[:,:,1:datasize-1])
        #sign_diff_true = sign.(vec_jmat_true[model] - vec_jmat_scaled[model][:,:,1:datasize-1])
        diff_gpr = vec_jmat_gpr[model] - vec_res[model].jmat[:,:,1:datasize-1]
        diff_true = vec_jmat_true[model] - vec_jmat_scaled[model][:,:,1:datasize-1]

        vec_cor_diff[model] = cor(vec(diff_gpr), vec(diff_true))
        vec_diff_gpr[model] = vec(diff_gpr)
        vec_diff_true[model] = vec(diff_true)
        ...
        vec_sign_diff_mean[model] = sum(sign.(vec_diff_mean_strength_gpr[model]) .== sign.(vec_diff_mean_strength_true[model]))/(datasize-1)
    end
    ...
    return (vec_diff_gpr=vec_diff_gpr, vec_diff_true=vec_diff_true, vec_diff_mean_strength_gpr=vec_diff_mean_strength_gpr, vec_diff_mean_strength_true=vec_diff_mean_strength_true, vec_cor_diff=vec_cor_diff, vec_sign_diff_mean=vec_sign_diff_mean)
end
...
function scatter_diff(vec_diff_gpr, vec_diff_true; model, nsp, datasize)
    diff_min = minimum(vec_diff_true[model])
    diff_max = maximum(vec_diff_true[model])
    rho = cor(vec_diff_gpr[model], vec_diff_true[model])
    bhat = [vec_diff_true[model] ones((datasize-1)nsp^2)]\vec_diff_gpr[model]

    sct = scatter(vec_diff_true[model], vec_diff_gpr[model], markersize=2, msw=0, xlab="ΔIS (theoretical)", ylab="ΔIS (estimated)", label="ρ = $(round(rho, digits=2))", legendfontsize=19, margin=0.3Plots.cm)
    #rho = cor(vec(jmat_scaled_fw1080[nozero,nozero,timepoint]), vec(jmat_RDE_fw10[:,:,timepoint]))
    plot!(sct, diff_min:0.01:diff_max, x -> x, c=:red, label=false)
    Plots.abline!(sct, bhat..., label=false)
